...
# Maintain three sections of the array using two indices, low and high: Strictly 'R's: array[:low] Unknown: array[
# low:high] Strictly 'G's: array[high:] Initially, low will be 0 and high will be len(array) - 1, since the whole
# array is unknown. As we iterate over the array, we'll swap any 'G's we see to the third section and decrement high.
# If we see an 'R', then we just need to increment low, since that's where it belongs. We can terminate once low
# crosses high. So we can gradually shrink our unknown section through the following algorithm:
def partition1(arr):
...
# Step1(Alternative):We'll initialize low and mid both to 0, and high to len(array) - 1 so that our unknown section
# is the whole array, as before. To maintain this invariant, we should do the following:
# Look at array[mid]:
